...
from Get_Image_Ratio import get_smallest_Image_Ratio
from Filter import *
...
def round_up_images(filter_img):
	...
	assert os.path.isdir(args.data_dir), "Couldn't find the dataset at {}".format(args.data_dir)
	for split in ['train', 'val', 'test']:
		data_directory = os.path.join(args.output_dir, '{}_data'.format(split))
		if os.path.exists(data_directory):
			...
			for filename in tqdm(filenames):
				image = Image.open(filename)
				image = apply_fundus_filter(filter_img, image)
				image.save(filename)
	

if __name__ == '__main__':
	
	image_filter = 'image_filter.jpg'
	filename = get_biggest_Image_Ratio()
	...
	'''
	fundus_filter = 'fundus_filter.jpg'
	filename = get_smallest_Image_Ratio(args.output_dir)
	Create_filter(filename, args.filter_dir, fundus_filter, False)
	filter_img = Image.open(os.path.join(args.filter_dir, fundus_filter.split('\\')[-1]))
	round_up_images(filter_img)
	''' #created to prevent bad filtering
